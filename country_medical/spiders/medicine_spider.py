...
class MedicineSpider(Spider):
    ...
    start_urls = []
    for i in range(1001, 1501): start_urls.append(url_pattern.format(i))  # 11111
    ...
    def parse(self, response):
        ...
        if a_el_list is None or len(a_el_list) == 0:
            writeFile(url=url, fileName="D:\workspace\country_medical\country_medical\exception-file\exception.txt")
        else:
            for a_el in a_el_list:
                u = "http://app1.sfda.gov.cn/datasearchcnda/face3/" + a_el.split(",")[1].replace("'", "")
                self.log("detail_url------------->>>{}".format(u))
                yield SplashRequest(url=u, callback=self.parse_item, dont_filter=True,
                                    args={"images": 0, 'timeout': 10, "wait": "5"})
        # else:
        #  self.parse_item(response)

    def start_requests(self):
        for url in self.start_urls: yield SplashRequest(url=url, dont_filter=True,
                                                        args={"images": 0, 'timeout': 10, "wait": "5"})
